...
for epoch in range(tr_epoch):
    for i, data in enumerate(data_trainer, 0):
        ...
        output2.backward()
        # if (i+1)%3==0 or i==(len(data_trainer)-1):
        #    optimizerD.step()
    ...
    if (epoch + 1) % 2 == 0:
        ...
        outprint.save('./results/' + str(epoch + 1) + version_p + 'th epoch lr00001SNGANanime256.png')
    ...
    if (epoch + 1) % 200 == 0:
        torch.save(generator.state_dict(), './models/' + str(epoch + 1) + version_p + 'lr00001SNGANanime256.pkl')
        torch.save(discriminator.state_dict(), './models/' + str(epoch + 1) + version_p + 'lr00001SNGANanime256.pkl')
