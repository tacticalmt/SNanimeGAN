...
learningr = 0.0002
times_batch = 2
back_version = ' v1007 '  # 与上一版区别是换了hinge loss  学习率提高   图像变128 层数改变
version_p = ' v1000b  '  #  新自制SN层
...
for epoch in range(tr_epoch):
    ...
    if current_iter % 1000 == 0:
        torch.save(generator.state_dict(),
                   './models/' + back_version + str(current_iter) + 'epoch' + version_p + 'SNGANanime256.pkl')
        torch.save(discriminator.state_dict(),
                   './models/' + back_version + str(current_iter) + 'epoch' + version_p + 'SNGANanime256.pkl')

np.save('./resultslog/'+'dislossall.npy', d_loss)
np.save('./resultslog/'+'genlossall.npy', g_loss)
