...
gpu = True
back_version = ' v1011 '  # 用官方SN  +attention  生成器用SN
version_p = ' v1000a  '  #
...
discriminator.load_state_dict(torch.load('./models/ v12 150 th  v10000a  attrisemiGAN_backbone.pkl'))
generator.load_state_dict(torch.load('./models/ v12 150 th  v10000a  attrisemiGAN_backbone.pkl'))
...
for epoch in range(tr_epoch):
    ...
    if (epoch + 1) % 2 == 0:
        torch.save(generator.state_dict(),
                   './models/' + back_version + str(epoch + 1) + 'epoch' + version_p + 'SNGANanimeGen'+str(img_size)+'.pkl')
        torch.save(discriminator.state_dict(),
                   './models/' + back_version + str(epoch + 1) + 'epoch' + version_p + 'SNGANanimeDis'+str(img_size)+'.pkl')
