...
gpu = True
back_version = ' v1014CP '  # 用官方SN  生成器用SN  判别器自制SN attention放在64*64层
version_p = ' v1000a  '  #  换个上采样方式  生成器有权重衰减
...
class Discr_net(nn.Module):  # 用这个
    def __init__(self, channel):
        ...
        self.CNN = nn.Sequential(OptimizedBlock(3, channel),  #64*64
                                 SNAttentionBlock(channel),
                                 Block(channel, channel * 2, downsample=True),  #32*32
...
discriminator.load_state_dict(torch.load('./models/ v1014 138epoch v1000a  SNGANanimeDis128.pkl'))
generator.load_state_dict(torch.load('./models/ v1014 138epoch v1000a  SNGANanimeGen128.pkl'))
